...
//テーブル作成
$sql="CREATE TABLE tb_mission_4_Saito"//テーブル作成
."("
...
if(isset($_POST['button']) and empty($_POST['editnumber2'])){   //送信ボタン押されhidden空の時
  ...
  $pdo=new PDO($dsn, $user, $password); //接続
  //テーブルに投稿内容挿入
  $sql=$pdo->prepare("INSERT INTO tb_mission_4_Saito(name, comment,date,pass)VALUES (:name,:comment,:date,:pass)");                //idは指定しない（idをauto_incrementにすればidの値指定しない時自動で振られる（連番で）ので）
...
if(isset($_POST['deletebutton'])){   //削除ボタン押されたとき
 if(!empty($_POST['delnumber']) and !empty($_POST['pass_del'])){  //番号とパスが送信された場合 
   ...
   /*******MySQLに接続********/
    ...
    $pdo=new PDO($dsn, $user, $password); //接続
  /******データを取り出す******/
    $sql='SELECT*FROM tb_mission_4_Saito'; //selectで全てのデータを取り出す
    $results= $pdo->query($sql);
    foreach ($results as $row){  //ループ処理(1つずつ取り出し）
     if($row['id']== $deleteNo and $row['pass']==$pass_del){ //パスと投稿番号一致
     $id=$deleteNo;
     $sql="delete from tb_mission_4_Saito where id=$id"; //idが一致する投稿を削除
     $result=$pdo->query($sql);
...
if(isset($_POST['editnumber'])){      //編集ボタン押されたとき
 if(!empty($_POST['editnumber']) and !empty($_POST['pass_edit'])){   //編集番号とパスが送信されたとき
   ...
   /*******MySQLに接続********/
    ...
    $pdo=new PDO($dsn, $user, $password); //接続
  /******データを取り出す******/
    $sql='SELECT*FROM tb_mission_4_Saito'; //selectで全てのデータを取得
    $results= $pdo->query($sql);
    foreach ($results as $row){  //ループ処理（1つずつ取り出し)
...
if(isset($_POST['button'])){   //送信ボタン押され時
 if(strlen($comment1)>0 and strlen($name1)>0 and !empty($_POST['editnumber2'])){ //各欄が空白でない時
       ...
       $pdo=new PDO($dsn, $user, $password); //接続
        ...
        /******データを取り出す******/
       $sql='SELECT*FROM tb_mission_4_Saito'; //selectですべてのデータを取得
       $results= $pdo->query($sql);
         foreach ($results as $row){  //ループ処理(1つずつ取り出し)
             if($row['id']==$editNo2){             //テキストボックスの値と投稿番号が一致する時
                ...
                $pasu=$pass_new;
                $sql="update tb_mission_4_Saito set name='$nm', comment='$kome', date='$hidzuke', pass='$pasu' where id ='$id'";//上書き
                $results=$pdo->query($sql);
              }//if
          }//foreach
...
$sql='SELECT*FROM tb_mission_4_Saito'; //selectですべてのデータ取得
$results= $pdo->query($sql);
